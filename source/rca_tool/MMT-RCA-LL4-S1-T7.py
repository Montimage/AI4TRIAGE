...
row += '}, "timestamp": "' + str(now) + '", "description": "Report from TPER user of depot F arrives. A report is created describing the LEPIDA network disruption and the offline status of depot | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
