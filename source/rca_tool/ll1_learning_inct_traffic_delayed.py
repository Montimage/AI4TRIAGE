...
row += '}, "timestamp": "' + str(now) + '", "description": "Railway#Stop and Bus#GenaralDelay reported by SOC of Slovenian Railways and SOC of LPP. Probable root-cause: Accident or damage on the road/ the bridge. Action: Evacuate the crowd | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
