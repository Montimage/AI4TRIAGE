...
row += '}, "timestamp": "' + str(now) + '", "description": "Excavation begins and the TLC cable is sheared off. The LEPIDA network notices partial disruption of service | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
