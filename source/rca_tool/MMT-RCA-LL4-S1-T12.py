...
row += '}, "timestamp": "' + str(now) + '", "description": "Continued rain, increased wind (exceeds safe threshold) and PM\'s anemometer activates shuttle operation stop. The wind-gauge sensor detects the strong wind and the SOC creates an event  | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
