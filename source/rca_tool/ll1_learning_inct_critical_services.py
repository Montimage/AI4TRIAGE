...
row += '}, "timestamp": "' + str(now) + '", "description": "Critical services degraded (heat energy, water and gas supply, public sewerage system) reported by SOC. Probable root-cause: Physical damages . Action: Check and fix | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
