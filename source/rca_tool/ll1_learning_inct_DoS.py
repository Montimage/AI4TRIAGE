...
row += '}, "timestamp": "' + str(now) + '", "description": "Multiple requests coming from several sources. Probable root-cause: Potential DDoS attack. Action: Blocking malicious related IP addresses | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
