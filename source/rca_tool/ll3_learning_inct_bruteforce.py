...
row += '}, "timestamp": "' + str(now) + '", "description": "Multiple log-in requests coming from the same source. Probable root-cause: Potential brute force attack. Action: Blocking the malicious related IP address | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
