...
row += '}, "timestamp": "' + str(now) + '", "description": "TPER Depot Z (UMP Urban Mobility Provider) stops being connected to the internet and intranet. LEPIDA SOC detects the event and creates an alert | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
