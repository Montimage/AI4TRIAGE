...
row += '}, "timestamp": "' + str(now) + '", "description": "The electricity provider notifies about potential electrical failures to its distribution network. The NOC creates alert of the event. | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
