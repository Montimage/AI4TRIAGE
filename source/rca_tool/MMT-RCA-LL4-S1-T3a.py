...
row += '}, "timestamp": "' + str(now) + '", "description": "Traffic queues in proximity of Z Deposit UMP. The local police received an alert about the traffic disease near the Z depot in order to intervene | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
