...
row += '}, "timestamp": "' + str(now) + '", "description": "USB stick inserted then malicious software connects periodically to a Command and Control Botnet Server. Probable root-cause: Malware infected. Action: Blocking botnet IP adresses- Removing the malware | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
