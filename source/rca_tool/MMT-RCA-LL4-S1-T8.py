...
row += '}, "timestamp": "' + str(now) + '", "description": "Power detachment of the cabin serving TPER\'s POP \"B\" due to heavy rain, (alternatively a cyber-attack drop off the TLC network). LEPIDA SOC detects the event and creates an alert | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
