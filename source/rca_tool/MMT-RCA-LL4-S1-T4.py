...
row += '}, "timestamp": "' + str(now) + '", "description": "Municipal Call center receives calls about the flooding.  The flooding is confirmed by a second event created by the Municipal Call Center and an alert is created on the LEPIDA network | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
