...
row += '}, "timestamp": "' + str(now) + '", "description": "Telecommunications and electricity cables disruption reported by SOC of TS and DKV of EL. Probable root-cause: Cables were disrupted due to accidents. Action: Verify the cables and connections | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
