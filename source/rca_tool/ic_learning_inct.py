...
def normalized_a (a): 
	...
	return a_normalized
#################################################################
mongoClient = pymongo.MongoClient("mongodb://localhost:27018") #tunnel to the server hosting the GUI
...
row += '}, "timestamp": "' + str(now) + '", "description": "Incident learned: Lower data rate + High CPU temperature in a group of devices. Probable root-cause: The devices are heated, possibly by a FIRE. Action: Activate the camera for visual verification | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
