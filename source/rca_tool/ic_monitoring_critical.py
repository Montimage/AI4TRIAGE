...
while 1:
    ...
    #print "Fire trigger at: " +str(now.year) + str(now.month) + str(now.day) + str(now.hour) + str(now.minute) + str(now.second) + str(now.microsecond)
    mongoClient = pymongo.MongoClient("mongodb://localhost:27017") #tunnel to the server hosting mongodb
    ...
    for raw_state in new_state_raw.find({}).sort([('timestamp', -1)]).limit(1): 
        ...
        print new_report
        now = datetime.datetime.now()
        #print "Fire detected at: " + str(now.year) + str(now.month) + str(now.day) + str(now.hour) + str(now.minute) + str(now.second) + str(now.microsecond)
        #new_state_raw.drop()
    time.sleep(5)
    print "Packet loss: " + str(pk_loss) + "%" 
