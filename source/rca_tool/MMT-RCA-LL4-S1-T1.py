...
row += '}, "timestamp": "' + str(now) + '", "description": "Intense rain. The local police office receives an alert about problems on the regular traffic flow | Source/ Dataset: '+ db_source +' | Confidence (number of relevant reports): ' + str(len(NB_CONN)+len(MS_DELAY)) +'"}'
indicators += '}'
print row 
print indicators
known_state = db_dest["data_knowledge_inc"]
